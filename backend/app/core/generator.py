...
class Generator:
    ...
    - 对于包含时间、奖项等结构化信息的问题，回答格式要求：
      1. 首先给出总结性回答，使用加粗格式（**内容**），例如："**周杰伦共拿过6次新人奖**"；
      2. 然后分点列出具体信息，每个点前面有编号，时间信息使用加粗格式（**时间**），例如："1. **2001年**：第一届全球华语歌曲排行榜\"最佳新人奖\""；
      3. 最后给出补充说明，使用加粗格式强调重要信息，例如："其中，**明确获奖的新人奖项共6项**（第2项仅为提名，不计入获奖次数）"；
